...
from .activations import Exp, ClippedSoftplus
from .loss_functions import QuadLoss 
...
class ZINBChromAutoEncoder(nn.Module):
    ...
    def __init__(
        ...
        latent_dim: int = 32,
        activation=nn.PReLU,
        final_activations=[Exp(), nn.Softplus(), nn.Sigmoid()],
        seed=4444,
    ):
        super().__init__()
        torch.manual_seed(seed)
...
class NBChromAutoEncoder(nn.Module):
    ...
    def __init__(
        ...
        latent_dim: int = 32,
        activation=nn.PReLU,
        final_activations=[Exp(), nn.Softplus()],
        seed=4444,
    ):
        super().__init__()
        torch.manual_seed(seed)
...
class NBAutoEncoder(nn.Module):
    ...
    def __init__(
        ...
        num_outputs: int = None,
        activation=nn.PReLU,
        final_activation=[Exp(), nn.Softplus()],
        seed=8482,
    ):
...
class PairedAutoEncoderSkorchNet(skorch.NeuralNet):
    def forward_iter(self, X, training=False, device="cpu"):
        ...
        for data in iterator:
            ...
            Xi = skorch.dataset.unpack_data(data)
...
def load_model(
    ...
    try:
        device_parsed = get_device(int(device))
    except (TypeError, ValueError):
        device_parsed = "cpu"

    # Infer input dim sizes if they aren't given
    if input_dim1 is None or input_dim1 <= 0:
        rna_genes = read_delimited_file(os.path.join(checkpoint, "rna_genes.txt"))
    ...
    if input_dim2 is None or (isinstance(input_dim2, int) and input_dim2 <= 0):
        atac_bins = read_delimited_file(os.path.join(checkpoint, "atac_bins.txt"))
    ...
    if checkpoint_basename.startswith("naive"):
        ...
        model_class = NaiveSplicedAutoEncoder
    else:
        logging.info(
            f"Inferred model with basename {checkpoint_basename} be normal (non-naive)"
        )
        model_class = AssymSplicedAutoEncoder

    spliced_net = None
    for hidden_dim_size in [16, 32]:
        try:
            spliced_net_ = SplicedAutoEncoderSkorchNet(
                module=model_class,
