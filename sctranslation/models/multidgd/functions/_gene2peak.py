...
def predict_perturbations_internal(model, rep, correction, dataset, modality_id, feature_id, n_epochs=1, device: str = "cuda"):
    ...
    for i in range(n_epochs):
        ...
        if model.train_set.modality_switch is not None:
            ...
            recon_loss_x = model.decoder.loss(y, x_perturbed, scale=dataset.library.to(device), gene_id=feature_id)
        loss = recon_loss_x
        loss.backward()
        rep_optimizer.step()
...
def predict_perturbations(model, testset, feature_id, split="test", new_data=False, steps=1, n_epochs=100, device: str = "cuda"):
    test_set = multiDGDDataset(
        testset,
    )
    ...
    try:
        ...
        model.test_rep.z = torch.nn.Parameter(torch.tensor(reps_original[:,:], device=device))
    torch.cuda.empty_cache()
