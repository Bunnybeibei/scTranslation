...
from .latent import GaussianMixture, GaussianMixtureSupervised
from .nn import Decoder
from .functions import train_dgd, set_random_seed, count_parameters, sc_feature_selection, learn_new_representations
from .functions._gene2peak import predict_perturbations
from .multidgd_dataloader import multiDGDDataset
...
class DGD(nn.Module):
    ...
    def __init__(
            self,
            train_set:multiDGDDataset=None,
            val_set:multiDGDDataset=None,
            test_set:multiDGDDataset=None,
        ...
        self._init_internal_attribs(covariates, model_name, save_dir, scaling, print_outputs) # needs to go? meta_label to data, dev_mode to train
        ...
        self.train_set = test_set if test_set is not None else train_set
    ...
    def _init_internal_attribs(self, covariates, model_name, save_dir, scaling, print_outputs):
        ...
        self._correction_variables = covariates
    ...
    def train(self, n_epochs=500, stop_with='loss', stop_after=10, train_minimum=50, developer_mode=False, train_loader=None, validation_loader=None):
        '''
    ...
    def save(self, save_dir=None, model_name=None):
        ...
        self._save_param_dict_to_file(str(save_dir/model_name))
    
    def _load_parameters(self):
        '''load model'''
        # load state dict
        checkpoint = torch.load(Path(self._save_dir)/(self._model_name+'.pt'),map_location=torch.device('cpu'))
    ...
    def load(cls,
        ...
        with open(Path(save_dir)/(model_name+'_hyperparameters.json'), 'r') as fp:
        ...
        model = cls(
                test_set=data,
                covariates=[x for x in data.data.obs.columns if 'covariate_' in x],
                scaling=scaling,
                save_dir=save_dir,
    ...
    def test(self, test_loader=None, n_epochs=20, external=False):
        '''high-level call of predict_new'''
        self.predict_new(test_loader=test_loader, n_epochs=n_epochs, external=False)
    ...
    def predict_new(self, test_loader=None, n_epochs=20, include_correction_error=True, indices_of_new_distribution=None, external=False):
        '''
    ...
    def gene2peak(self, gene_name, testset, gene_ref=None, device='cuda'):
        '''
        ...
        predictions_original_gene, predictions_upregulated_gene, indices_of_interest_gene = predict_perturbations(self, testset, gene_id, device=device)
        predicted_changes = [(predictions_upregulated_gene[i].to(device) - predictions_original_gene[i]) for i in range(len(self.train_set.modalities))]
