...
class OutputModule(nn.Module):
    ...
    def __init__(self,
        ...
        elif modality in ['ADT','adt']:
            self.distribution = MultivariateNormal_Layer(out_features)
        else: 
...
class MultivariateNormal_Layer(nn.Module):
    '''
    This is the Negative Binomial version of the OutputModule distribution layer.
    ...
    def __init__(self, out_features):
        super(MultivariateNormal_Layer, self).__init__()
        self.loc_layer = nn.Linear(out_features, out_features, bias=False)
        self.cov_layer = nn.Linear(out_features, out_features, bias=False)
    
    def forward(self, x):
        '''forward pass through the NB layer'''
        loc = self.loc_layer(x)
        logvar = self.cov_layer(x)
        # map logvar from 1-dimensional to diagonal covariance matrix
        cov = torch.diag_embed(torch.exp(logvar))
        
        self.dist = torch.distributions.MultivariateNormal(loc=loc,
                                                           covariance_matrix=cov)
        return loc
    
    def log_prob(self,model_output,target,scaling_factor,gene_id=None,mask=None):
        '''returns the log-prob of the multivariate normal layer'''
        if gene_id is not None:
            logprob = self.dist.log_prob(target[:,gene_id])
        else:
            logprob = self.dist.log_prob(target)
        if mask is not None:
            logprob[mask,:] = 0 # mask must be a boolean 1D tensor
        return logprob[:, None]  # Add a dimension to match the output shape
