...
class EncoderModel(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # List of modalities present in the data. ``in_dim`` is the source of
        # truth (the YAML may pre-configure encoder/decoder kwargs only for
        # rna+atac, but the actual second modality can be ADT). We copy the
        # atac template into any missing modality (e.g. adt) so the user can
        # keep a single encoder YAML.
        if multimodal:
            self.modality_list = list(self.in_dim.keys())
            for mod in self.modality_list:
                if mod not in encoder_kwargs:
                    template_mod = "atac" if "atac" in encoder_kwargs else next(
                        (m for m in encoder_kwargs if m != "rna"), None
                    )
                    if template_mod is None:
                        raise KeyError(
                            f"No encoder_kwargs template available for modality '{mod}'."
                        )
                    encoder_kwargs[mod] = encoder_kwargs[template_mod]
        ...
        if not self.multimodal:
            ...
            for mod in self.modality_list:
                ...
                # ADT uses a Gaussian likelihood, so it needs an *independent*
                # variance head with the same architecture as the mean decoder
                # (not the same weights). Sharing the dict caused
                # ``decoder_var[mod](x)`` to return the mean.
                if mod == "adt":
                    decoder_var_modules[mod] = MLP(**encoder_kwargs[mod])
            self.encoder = torch.nn.ModuleDict(self.encoder)
            self.decoder = torch.nn.ModuleDict(self.decoder)
            if decoder_var_modules:
                self.decoder_var = torch.nn.ModuleDict(decoder_var_modules)
        ...
        if not self.multimodal:
            ...
            for mod in mu_hat:
                if mod == "rna":
                    ...
                    cov = torch.diag_embed(mu_hat[mod]['logvar'].exp().to(X[mod].device))
                    px = MultivariateNormal(loc=mu_hat[mod]['mu'].to(X[mod].device), covariance_matrix=cov)
                    loss -= px.log_prob(X[mod]).mean()
                else:
    ...
    def encode(self, batch):
        ...
        if not self.multimodal:
            ...
            z = {}
            for mod in self.modality_list:
                z_mod = self.encoder[mod](batch["X_norm"][mod].to(self.device).float())
                z[mod] = z_mod
    ...
    def decode(self, x, size_factor):
        ...
        if not self.multimodal:
            ...
            for mod in self.modality_list:
                ...
                if mod == "rna" or (mod == "atac" and not self.is_binarized):
                    ...
                    x_var = self.decoder_var[mod](x[mod])
                    mu_hat[mod] = {}
                    mu_hat[mod]['mu'] = x_mod
                    mu_hat[mod]['logvar'] = x_var
        return mu_hat
