...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        self.save_interval = save_interval #// dist.get_world_size()
        ...
        self.lr_anneal_steps = lr_anneal_steps // dist.get_world_size()
        print(f"lr_anneal_steps: {self.lr_anneal_steps}")
    ...
    def run_loop(self):
        ...
        while not self.lr_anneal_steps or self.step + self.resume_step < self.lr_anneal_steps:
            ...
            # print(f"The first 5 elements of the batch: {batch['video'][0]}")
            # # # print shape
            ...
            # 梯度同步检测
            if self.step % 10 == 0:
                grad_norm = th.norm(th.stack([th.norm(p.grad.detach(), 2) 
                                for p in self.model.parameters() if p.grad is not None]), 2)
                # print(f"[Rank {dist.get_rank()}] Gradient norm: {grad_norm.item():.4f}")

            # 日志记录同步
            if dist.get_rank() == 0:
                wandb_log = {
                    'loss': loss["loss"].mean().item(),
                }
                
                if self.step % self.log_interval == 0:
                    log = logger.get_current()
                    additional_metrics = {
                        'grad_norm': log.name2val.get("grad_norm", 0),
                        'loss_q0': log.name2val.get("loss_q0", 0),
                        'v_grad': log.name2val.get("grad_norm_v", 0),
                        'a_grad': log.name2val.get("grad_norm_a", 0)
                    }
                    wandb_log.update(additional_metrics)
                    logger.dumpkvs()
                    wandb.log(wandb_log)
            ...
            self.step += 1
            if self.step % self.log_interval == 0:
                logger.log(f"Step {self.step} in rank {dist.get_rank()} finished.")
            # print(f"Step {self.step} in rank {dist.get_rank()} finished.")
    ...
    def save(self):
        def save_checkpoint(rate, params):
            ...
            if dist.get_rank() == 0:
                ...
                with bf.BlobFile(bf.join(self.output_dir, filename), "wb") as f:
                    th.save(state_dict, f)
                logger.log(f"saving model {rate} in {filename}...")
        if dist.get_rank() == 1:
            print(f"Saving model {self.step} in {self.output_dir}...")
        ...
        if dist.get_rank() == 0:
            with bf.BlobFile(
                bf.join(self.output_dir, f"opt{(self.step+self.resume_step):06d}.pt"),
                "wb",
            ) as f:
                th.save(self.opt.state_dict(), f)

        logger.info(f"Process {dist.get_rank()} reached the barrier.")
        dist.barrier()
        logger.info(f"Process {dist.get_rank()} passed the barrier.")
...
def find_resume_checkpoint(output_dir):
    ...
    filename = "model*.pt"
    max_step = 0
    for name in glob.glob(os.path.join(output_dir, filename)):
        step = int(name[-9:-3])
        max_step = max(max_step, step)
    if max_step:
        path = bf.join(output_dir, f"model{(max_step):06d}.pt")
