...
import muon as mu
import torch
from .Autoencoder.data.utils import normalize_expression, compute_size_factor_lognorm
from scipy.sparse import csr_matrix
from .DiffusionBackbone.multimodal_datasets import load_data_cell

class RNAseqLoader:
    """Class for RNAseq data loader."""
    def __init__(
        self,
        data_r: sc.AnnData=None,
        data_modal2: sc.AnnData=None,
        modal2: str=None,
        layer_key: str=None,
        covariate_keys=None,
        subsample_frac=1,
        ...
        if data_r is not None:
            self.modality_list.append('rna')
            adata['rna'] = data_r
        if data_modal2 is not None:
            modality2_key = {'a': 'atac', 'p': 'adt'}.get(modal2)
            if modality2_key is None:
                raise ValueError(f"Unsupported modal2 '{modal2}'; expected 'a' or 'p'.")
            self.modality_list.append(modality2_key)
            adata[modality2_key] = data_modal2

        # Joint filtering: a cell is kept only if every loaded modality has a
        # non-null ``cell_type`` for it. Filtering each modality independently
        # produces mismatched sample counts when one modality is missing
        # labels (very common for ADT-only metadata).
        if len(self.modality_list) > 1:
            valid_per_mod = [
                adata[m].obs.index[~adata[m].obs["cell_type"].isna()]
                for m in self.modality_list
            ]
            common = valid_per_mod[0]
            for ids in valid_per_mod[1:]:
                common = common.intersection(ids)
            for m in self.modality_list:
                adata[m] = adata[m][common].copy()
        elif self.modality_list:
            m = self.modality_list[0]
            adata[m] = adata[m][~adata[m].obs["cell_type"].isna()].copy()
        ...
        if not self.multimodal:
            ...
            self.X = {}
            for mod in self.modality_list:
                if isinstance(adata[mod].layers[layer_key], csr_matrix):
                    self.X[mod] = torch.Tensor(adata[mod].layers[layer_key].todense())
                else:
                    self.X[mod] = torch.Tensor(adata[mod].layers[layer_key])
        ...
        for cov_name in covariate_keys:
            ...
            try:
                ...
                cov_str = cov.astype(str)
                unique_cov = np.unique(cov_str)
...
def load_training_data(train_data, args,dev="cuda:0"):
    data = load_data_cell(
