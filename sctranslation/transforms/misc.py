...
class Compose(BaseTransform):
    ...
    def __init__(self, *transforms: Tuple[BaseTransform, ...], **kwargs):
        ...
        if failed_list:
            failed_types_str = ", ".join(
                f"{type(transform).__name__}" for transform in failed_list
            )
            raise TypeError(
                f"All transform objects must inherit from BaseTransform. The following have incorrect types: {failed_types_str}"
            )
    ...
    def transform(self, ad: sc.AnnData):
        pass

    def __call__(self, data, data_type: str = None) -> None:
        for transform in self.transforms:
            data = transform(data, data_type=data_type)
        return data
